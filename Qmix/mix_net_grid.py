...
class QMIX_Net(nn.Module): # 모든 에이전트들의 Q_value 값들을 받아서 total Q_value 을 만드는 클래스
    def __init__(self, args):
        super(QMIX_Net, self).__init__()
        self.N = args.N #에이전트 수
    ...
    def forward(self, q, s):
        # q.shape(batch_size, episode_len, N) #에피소드 단위로 데이터를 만들어서 input으로 주려는 것 같은데, gridworld 에서는 어차피 전체 step이 정해져 있으니까 상관없음
        # q는 각 에이전트의 max(Q_value) 값들을 나타낸 것이다.
        # s.shape(batch_size, episode_len,state_dim) --> statd_dim 은 한 줄로 편게 아닐까 함
