...
def main():
    ...
    for ep in range(args.total_ep):
        ...
        for agent in env.agent_iter():
            ...
            #book process after all agents took actions
            ...
            if (((iteration_number) % (args.n_predator1 + args.n_predator2 + args.n_prey)) == 0) and (iteration_number > 0):
                ...
                #마지막 스텝이 아니라면, truncation 하지 않기 때문에 다르게 해주어야함, 마지막 step이 아닌 곳에 대해서만 shared_book에 넣어주는 작업을 해야 한다.
                ...
                # # 그렇기 때문에 단순히 모델만 돌리고자 할때는 이 부분 없어도 된다.
                # else:
        ...
        if (ep % args.ep_save) ==0 :
            for i in range(len(madqn.gdqns)) :
                ...
                th.save(madqn.gdqn_targets[i].state_dict(), 'model_save/' + 'model_target_' + str(i) + '_ep' + str(ep)+ '.pt')
