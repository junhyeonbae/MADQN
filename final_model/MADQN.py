...
class MADQN():  # def __init__(self,  dim_act, observation_state):
    ...
    def replay(self):
        for _ in range(args.replay_times):
            ...
            q_values, _,_,_,_,_,_ = self.gdqn(observations.unsqueeze(0), adj.unsqueeze(0), book[0].detach().to(self.device))
            q_values = q_values[actions]


            next_q_values, _,_,_,_,_,_ = self.gdqn_target(next_observations.unsqueeze(0), adj.unsqueeze(0), book_next[0].detach().to(self.device))
